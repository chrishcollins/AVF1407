...
//Rare exceptions to indicate a bug in the _weather code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_weather = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_weather]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.weather.module"
